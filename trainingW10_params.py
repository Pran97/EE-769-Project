...
class MyTradingFunctions():
    ...
    def getInstrumentFeatureConfigDicts(self):
        ...
        mom2Dict = {'featureKey': 'mom_10',
                   ...
                   'params': {'period': 20,
                              'featureName': 'stockVWAP'}}
        sdevDict = {'featureKey': 'sdev_20',
                    'featureId': 'moving_sdev',
                    'params': {'period': 20,
                               'featureName': 'stockVWAP'}}
        rsi1 = {'featureKey': 'rsi2',
                  'featureId': 'rsi',
                  'params': {'period': 10,
                             'featureName': 'stockVWAP'}}
        rsi4 = {'featureKey': 'rsi8',
                  'featureId': 'rsi',
                  'params': {'period': 20,
                             'featureName': 'stockVWAP'}}


        return [mom1Dict, mom2Dict,rsi1,rsi4,sdevDict]
    ...
    def getPrediction(self, time, updateNum, instrumentManager, predictions):
        ...
        sd = lookbackInstrumentFeatures.getFeatureDf('sdev_20')
        cs=lookbackInstrumentFeatures.getFeatureDf('rsi2')
        cs2=lookbackInstrumentFeatures.getFeatureDf('rsi8')
        #Get latest factor values
        factorValues = (sd/(mom1-mom2)).iloc[-1]
        ...
        if len(mom1.index) > 1:
            oldFactorValues = ((mom1-mom2)/sd).iloc[-2]
